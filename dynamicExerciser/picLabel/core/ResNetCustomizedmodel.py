...
from dynamicExerciser.picLabel.core import common as common, NewVgg16 as VGG16, utils as utils
...
class CustomizedModel(object):
    ...
    def __build_network(self, input_data, input_labels, trainable): 
        ...
        # 7个resnet网络分别训练
        # sbbox_resnet_output = VGG16(conv_sbbox, input_labels, 'resnet_sbbox').model_resnet
        # lbbox_resnet_output = VGG16(conv_lbbox, input_labels, 'resnet_lbbox').model_resnet
        # mbbox_resnet_output = VGG16(conv_mbbox, input_labels, 'resnet_mbbox').model_resnet
        # smbbox_resnet_output = VGG16(conv_smbbox, input_labels, 'resnet_smbbox').model_resnet
        # slbbox_resnet_output = VGG16(conv_slbbox, input_labels, 'resnet_slbbox').model_resnet
        # mlbbox_resnet_output = VGG16(conv_mlbbox, input_labels, 'resnet_mlbbox').model_resnet
        # smlbbox_resnet_output = VGG16(conv_smlbbox, input_labels, 'resnet_smlbbox').model_resnet


        all_softmax_output = lbbox_output['output'] + mbbox_output['output'] + sbbox_output['output'] + smbbox_output['output'] + slbbox_output['output'] + mlbbox_output['output'] + smlbbox_output['output']
        all_cost = lbbox_output['cost'] + mbbox_output['cost'] + sbbox_output['cost'] + smbbox_output['cost'] + slbbox_output['cost'] + mlbbox_output['cost'] + smlbbox_output['cost']
        ...
        # RESNET的输出
        # all_resnet_softmax_output = lbbox_resnet_output['output'] + mbbox_resnet_output['output'] + sbbox_resnet_output['output'] + smbbox_resnet_output['output'] + slbbox_resnet_output['output'] + mlbbox_resnet_output['output'] + smlbbox_resnet_output['output']
        # all_resnet_cost = lbbox_resnet_output['cost'] + mbbox_resnet_output['cost'] + sbbox_resnet_output['cost'] + smbbox_resnet_output['cost'] + slbbox_resnet_output['cost'] + mlbbox_resnet_output['cost'] + smlbbox_resnet_output['cost']

        # prediction_resnet_labels = tf.argmax(all_resnet_softmax_output, axis=1, name="output")
        # correct_resnet_prediction = tf.equal(prediction_resnet_labels, read_labels)
        # accuracy_resnet = tf.reduce_mean(tf.cast(correct_resnet_prediction, tf.int32))
        # resnet_correct_times_in_batch = tf.reduce_sum(tf.cast(correct_resnet_prediction, tf.int32))


        # 总的输出
        # all_softmax_output = all_softmax_output + all_resnet_softmax_output
        # all_cost = all_cost + all_resnet_cost
        # all_prediction_labels = tf.argmax(all_softmax_output, axis=1, name="output")
        # all_correct_prediction = tf.equal(prediction_labels, read_labels)
        # correct_times_in_batch = tf.reduce_sum(tf.cast(correct_prediction, tf.int32))
        ...
        return dict(
            ...
            # m_resnet_optimize=mbbox_resnet_output['optimize'],
            # l_resnet_optimize=lbbox_resnet_output['optimize'],
            # s_resnet_optimize=sbbox_resnet_output['optimize'],
            # sm_resnet_optimize=smbbox_resnet_output['optimize'],
            # sl_resnet_optimize=slbbox_resnet_output['optimize'],
            # ml_resnet_optimize=mlbbox_resnet_output['optimize'],
            # sml_resnet_optimize=smlbbox_resnet_output['optimize'],
            # TODO
            # bbox output
            # lbbox_out
            l_out=dict(
            ...
            # l_resnet_out=dict(
            #     cost=lbbox_resnet_output['cost'],
            #     correct_prediction=lbbox_resnet_output['correct_prediction'],
            #     correct_times_in_batch=lbbox_resnet_output['correct_times_in_batch'],
            #     prediction_labels=lbbox_resnet_output['prediction_labels']
            # ),
            # mbbox_out
            m_out=dict(
            ...
            # m_resnet_out=dict(
            #     cost=mbbox_resnet_output['cost'],
            #     correct_prediction=mbbox_resnet_output['correct_prediction'],
            #     correct_times_in_batch=mbbox_resnet_output['correct_times_in_batch'],
            #     prediction_labels=mbbox_resnet_output['prediction_labels']
            # ),
            # sbbox_out
            s_out=dict(
            ...
            # s_resnet_out=dict(
            #     cost=sbbox_resnet_output['cost'],
            #     correct_prediction=sbbox_resnet_output['correct_prediction'],
            #     correct_times_in_batch=sbbox_resnet_output['correct_times_in_batch'],
            #     prediction_labels=sbbox_resnet_output['prediction_labels']
            # ),
            sm_out=dict(
            ...
            # sm_resnet_out=dict(
            #     cost=smbbox_resnet_output['cost'],
            #     correct_prediction=smbbox_resnet_output['correct_prediction'],
            #     correct_times_in_batch=smbbox_resnet_output['correct_times_in_batch'],
            #     prediction_labels=smbbox_resnet_output['prediction_labels']
            # ),
            sl_out=dict(
            ...
            # sl_resnet_out=dict(
            #     cost=slbbox_resnet_output['cost'],
            #     correct_prediction=slbbox_resnet_output['correct_prediction'],
            #     correct_times_in_batch=slbbox_resnet_output['correct_times_in_batch'],
            #     prediction_labels=slbbox_resnet_output['prediction_labels']
            # ),
            ml_out=dict(
            ...
            # ml_resnet_out=dict(
            #     cost=mlbbox_resnet_output['cost'],
            #     correct_prediction=mlbbox_resnet_output['correct_prediction'],
            #     correct_times_in_batch=mlbbox_resnet_output['correct_times_in_batch'],
            #     prediction_labels=mlbbox_resnet_output['prediction_labels']
            # ),
            sml_out=dict(
            ...
            # sml_resnet_out=dict(
            #     cost=smlbbox_resnet_output['cost'],
            #     correct_prediction=smlbbox_resnet_output['correct_prediction'],
            #     correct_times_in_batch=smlbbox_resnet_output['correct_times_in_batch'],
            #     prediction_labels=smlbbox_resnet_output['prediction_labels']
            # ),
            # fin_out
            fin_nsl_out=dict(
                cost=all_cost,
            ...
            # fin_out_resnet=dict(
            #     cost=all_resnet_cost,
            #     correct_prediction=correct_resnet_prediction,
            #     correct_times_in_batch=resnet_correct_times_in_batch,
            #     prediction_labels=prediction_resnet_labels
            # ),
            # all_fin_out=dict(
            #     cost=all_cost,
            #     correct_prediction=all_correct_prediction,
            #     correct_times_in_batch=all_correct_times_in_batch,
    ...
    def mergebox(self, conv_sbbox=None, conv_mbbox=None, conv_lbbox=None, name=None):
        if conv_lbbox != None:
            ...
            # conv_lbbox = conv_lbbox
        else:
            # conv_lbbox = tf.zeros([10, 32, 32, self.in_channel])  # TODO 硬写入的形状，后面可能会改
            conv_lbbox = np.zeros_like(conv_mbbox, dtype='float32')
            # conv_lbbox = tf.zeros(tf.shape(conv_lbbox))
        
        if conv_mbbox != None:
            conv_mbbox = common.convolutional(conv_sbbox, filters_shape=(3, 3, self.in_channel, self.in_channel),
                                              trainable=True, name=name+'_downsample', downsample=True)
            conv_mbbox = conv_mbbox
        else:
            # conv_mbbox = tf.zeros([10, 32, 32, self.in_channel])
            conv_mbbox = np.zeros_like(conv_mbbox, dtype='float32')
            # conv_mbbox = tf.zeros(tf.shape(conv_mbbox))
        ...
        if conv_sbbox != None:
            ...
            # conv_sbbox = common.convolutional(conv_sbbox, filters_shape=(3, 3, self.in_channel, self.in_channel), 
            #                                     trainable=True, name=name+'_downsample', downsample=True)
            # conv_sbbox = conv_sbbox
        else:
            # conv_sbbox = tf.zeros([10, 32, 32, self.in_channel])
            conv_sbbox = np.zeros_like(conv_mbbox, dtype='float32')
            # conv_sbbox = tf.zeros(tf.shape(conv_sbbox))
    ...
    def mergebox_new(self, conv_sbbox=None, conv_mbbox=None, conv_lbbox=None, name=None):
        if name == 'merge_sm':
            conv_mbbox = common.convolutional(conv_mbbox, filters_shape=(3, 3, self.in_channel, self.in_channel), trainable=True, name=name + 'convm0')
            conv_sbbox = common.convolutional(conv_sbbox, filters_shape=(3, 3, self.in_channel, self.in_channel),
    ...
    def vote_result(self, input_bbox_dict):
        vgg_softmax_output = input_bbox_dict['lbbox_output']['output'] + input_bbox_dict['sbbox_output']['output'] + \
                            ...
                            input_bbox_dict['sbbox_resnet_output']
