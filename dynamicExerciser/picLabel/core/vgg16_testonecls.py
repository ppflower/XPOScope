...
class VGG16(object):
    ...
    def __init__(self, ldata, mdata, sdata, input_labels, name):
        # 预训练参数
        ...
        try:
            self.model = self.__build_network(ldata, mdata, sdata, input_labels, name)
        except:
            raise NotImplementedError("Can not build up our customized network!")

    def __build_network(self, ldata, mdata, sdata, input_labels, name):
        ...
        with tf.variable_scope(name):
            ...
            conv1_1 = common.convolutional(input_data=ldata,
                                           filters_shape=(3, 3, in_channel, out_channel),
                                           trainable=True, name='conv1_1')
            conv1_2 = common.convolutional(input_data=conv1_1,
                                           filters_shape=(3, 3, out_channel, out_channel),
                                           trainable=True, name='conv1_2')
            finl = common.convolutional(input_data=conv1_2,
                                        filters_shape=(3, 3, out_channel, out_channel),
                                        trainable=True, name='conv1_3')

            conv2_1 = common.convolutional(input_data=mdata,
                                           filters_shape=(3, 3, in_channel, out_channel),
                                           trainable=True, name='conv2_1')
            conv2_2 = common.convolutional(input_data=conv2_1,
                                           filters_shape=(3, 3, out_channel, out_channel),
                                           trainable=True, name='conv2_2')
            finm = common.convolutional(input_data=conv2_1,
                                        filters_shape=(3, 3, out_channel, out_channel),
                                        trainable=True, name='conv2_3')

            conv3_1 = common.convolutional(input_data=sdata,
                                           filters_shape=(3, 3, in_channel, out_channel),
                                           trainable=True, name='conv3_1')
            conv3_2 = common.convolutional(input_data=conv3_1,
                                           filters_shape=(3, 3, out_channel, out_channel),
                                           trainable=True, name='conv3_2')
            fins = common.convolutional(input_data=conv3_2,
                                        filters_shape=(3, 3, out_channel, out_channel),
                                        trainable=True, name='conv3_3')

            fins = tf.nn.max_pool(fins, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], 
                                    padding='SAME', name='pool5')
            fins = tf.nn.max_pool(fins, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], 
                                    padding='SAME', name='pool5')
            finm = tf.nn.max_pool(finm, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], 
                                    padding='SAME', name='pool5')
            fin = finl + fins + finm

            fin = common.convolutional(input_data=fin,
                                       filters_shape=(3, 3, in_channel, out_channel),
