...
def traffic_listening_server(url_traffic_map, url_page_index_map):
    ...
    ip_port = ('127.0.0.1', config.ALIPAY_TRANSFER_PORT)
    server.bind(ip_port)
    server.listen(100)
...
def link(sock: socket, addr, url_traffic_map, url_page_index_map):
    ...
    while len(recv_data) != 0:
        all_data += recv_data
        if recv_data.endswith(b'Bye'):
            break
        recv_data = sock.recv(config.BUFFER_SIZE)
    all_str = str(all_data, encoding='utf-8')
    resp_str = all_str[:-len("Bye")]
    try:
        recv_json = json.loads(resp_str.strip())
    except json.JSONDecodeError:
        print("JSON DECODE ERROR")
        return
    opt_type = recv_json['opt_type']

    if opt_type == 1 or opt_type == '1':
        data_str = recv_json['data']
        if isinstance(data_str, str):
            data_str = data_str.strip().replace(' ', '').replace('\n', '').replace('\r', '')
        try:
            data_json = json.loads(data_str.strip())
        except json.JSONDecodeError:
            print("JSON DECODE ERROR 2")
            return

        privacy_data = data_json
...
if __name__ == "__main__":
    ...
    for mini_app_type in config.All_Mini_App_Types:
        ...
        for i in range(mini_apps_start_num + 1, len(mini_app_names)):
            ...
            traffic_listen_process = multiprocessing.Process(target=run_traffic_listening_server, args=(
                url_traffic_map, url_page_index_map,
            ))
